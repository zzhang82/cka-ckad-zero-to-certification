...
def main() -> int:
    errors: list[str] = []
    catalog_path = ROOT / "_project" / "data" / "resources" / "catalog.json"
    ...
    if not isinstance(resources, list) or not resources:
        errors.append("_project/data/resources/catalog.json: resources must be a non-empty array")
        resources = []
    ...
    for index, resource in enumerate(resources):
        label = f"_project/data/resources/catalog.json resources[{index}]"
    ...
    for week_path in week_files:
        ...
        if missing:
            ...
            continue
        if week["id"] != week_path.stem or week["id"] not in VALID_WEEKS:
            errors.append(f"{rel}: id must match filename and be week-00 through week-12")
    ...
    print(
        f"Validated {len(resources)} catalog resources, {len(VALID_WEEKS)} public week entrances, "
        f"and {len(week_files)} machine week contract(s)."
    )
    return 0
