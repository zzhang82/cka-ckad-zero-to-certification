...
Status: **historical planning baseline**. The current learner entrance is [the weekly program](../../weeks/README.md), and the implemented directory/command contract is [Repository Architecture](../ARCHITECTURE.md). Later verified implementation evidence overrides provisional wording below.
...
The training consequence is diagnostic-driven compression. Familiar concepts receive short retrieval checks, while command fluency, evidence-driven diagnosis, cluster internals, and timed execution receive repeated hands-on work. See the [reference learner](REFERENCE_LEARNER.md).
...
- passed two different unseen internal mocks under the [readiness rules](../quality/ACCEPTANCE_STANDARD.md);
...
Current verified V1 design:

- canonical shell: Ubuntu 24.04 under WSL2; the maintainer distro name is not a public requirement;
- container runtime: existing native WSL Docker Engine;
- first local cluster engine: `kind`, live-tested with the pinned Week 0 image;
- pinned `kubectl`, `kind`, Helm 4, Helm 3 compatibility, and `yq` clients in WSL;
...
```text
README.md             # learner router
weeks/                # weekly directions and curated resources
resources/            # cross-week learner index
docs/en/              # prerequisites and setup help
study                 # stable learner command
_project/             # scripts, graders, templates, data, policy, evidence
learner-state/        # ignored private study records
.state/               # ignored runtime and kubeconfig
dist/                 # generated; never canonical
```
